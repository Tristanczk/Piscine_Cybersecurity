...
def process_payload(payload, verbose):
    ...
    if header == "QUIT":
        ...
        message = f"User entered the following password to connect: '{payload[5:]}'"
    elif header == "530":
...
def sniffing_ftp(verbose):
    scapy.sniff(filter="tcp and port 21", prn=lambda x: process_ftp_packet(x, verbose))

def main():
    args = parse_arguments()
    sniffing_thread = threading.Thread(target=sniffing_ftp, args=(args.verbose,))
